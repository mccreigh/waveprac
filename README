...
This package is not ready to build. I'm using devtools to develop it
https://github.com/hadley/devtools
Once you download and unpack it, command line:
`$ R CMD INSTALL hadley-devtools-1234567`

In R:
`require(devtools)`
then see examples.r for loading the development package and data and running some things. 

I'm not sure if DESCRIPTION will force its dependencies. The current package depenedencies are
ggplot2
plyr
multicore (if you want to use more than 1 core on a multicore machine)
...
Some notes on where things stand:
enhanced.warm simulation is not right.
enhanced.warm prediction is highly experimental.
